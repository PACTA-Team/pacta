...
-- +goose Up
-- Add pending_activation status to users CHECK constraint
...
-- Create new table with updated constraint including pending_activation
...
INSERT INTO users_new (id, name, email, password_hash, role, status, last_access, created_at, updated_at, deleted_at)
SELECT id, name, email, password_hash, role, status, last_access, created_at, updated_at, deleted_at FROM users WHERE status IN ('active', 'inactive', 'locked', 'pending_email', 'pending_approval', 'pending_activation');
