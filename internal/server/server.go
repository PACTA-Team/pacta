...
func Start(cfg *config.Config, staticFS fs.FS) error {
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Head("/api/documents/temp/{key}", h.verifyTempDocument)
			// Allow GET for convenience (fetch temp file directly)
			r.Get("/api/documents/temp/{key}", h.serveTempDocument)
		...
		r.Group(func(r chi.Router) {
			...
			r.Post("/api/upload/temp", h.uploadTempDocument)
			r.Delete("/api/documents/temp/{key}", h.cleanupTempDocument)
